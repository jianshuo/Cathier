...
| 2026-05-11 | 五彩斑斓的黑 — animated MeshGradient background | User-requested feature. Dark mode only: 3×3 MeshGradient whose 9 vertices are each tinted with a near-black hue (<11% lightness) from one of the 9 emotion categories. Center vertex drifts in a Lissajous path (~3-min cycle). Light mode unaffected (cathierBackground). Reduce Motion: static gradient. Applied to all 5 main NavigationStack content areas (TodayView, JournalView, SettingsView, FriendFeedView, PlazaView). Intentional exception to the "no gradient backgrounds" anti-pattern — the colors are all effectively black so the surface still reads as dark and calm. |
